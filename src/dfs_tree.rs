...
use crate::{ constants::{ORANGE, BLACK, RED, GRAY}, data_structs::ColoredGraph, data_structs::Graph, data_structs::Node, hashed_list::HashedList };

pub fn create_tree(g: &mut ColoredGraph, c: usize, cycle: Vec<usize>) -> (HashedList<usize>, HashedList<usize>) {
    // let mut postorder = Vec::new();
    // let mut inorder = Vec::new();
    let mut postorder = HashedList::<usize>::new(Some(vec![]));
    let mut inorder = HashedList::<usize>::new(Some(vec![]));
...
pub fn create_dfs_order_cycle(v: u32, g: &mut ColoredGraph) -> (HashedList<u32>, Option<u32>) {
    ...
    while let Some(&u) = stack.last() {
        let c = g.get_color(u as usize);
        
        if c != GRAY && c != BLACK {
            g.set_color(u as usize, GRAY);
            
            for &w in g.graph.get_successors(u as usize) {
                if cycle && w == v as usize {
...
pub fn create_dfs_order(v: u32, g: &mut ColoredGraph) -> HashedList<u32> {
