...
use crate::{data_structs::ColoredGraph, constants::{RED, ORANGE, BLACK}, cycle_utils::cycle_distance, dfs_tree::create_tree, hashed_list::HashedList};

fn finish_subtree(g: &mut ColoredGraph, v: usize) {
...
fn calculate_cmax(g: &mut ColoredGraph, cycle: &[usize], postorder: &HashedList<usize>, inorder: &HashedList<usize>, i: usize) -> (bool, Option<u32>) {
    ...
    let check_backedge = |w: usize, u: usize, postorder: &HashedList<usize>| -> bool {
    ...
    (false, g.graph.get_property(cycle[i], "max".to_string()))
}

pub fn find_paths(g: &mut ColoredGraph, cycle: &[usize]) -> (bool, Vec<u32>) {
    let mut result = Vec::new();
    for i in 0..cycle.len() {
        let c = cycle[i];
        let (postorder, inorder) = create_tree(g, c, cycle.to_vec());
        let (cover, value) = calculate_cmax(g, cycle, &postorder, &inorder, i);
        if cover {
